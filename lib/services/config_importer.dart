...
class ConfigImporter {
  ...
  Future<bool> syncErrorTypeOutlinesFromConfig() async {
    try {
      // 先尝试从应用目录读取，再从资源文件读取
      final appConfigPath = await _getConfigFilePath('error_type_outlines.json');
      String? jsonString;
  ...
  Future<bool> importAllConfig({bool clearExisting = true}) async {
    try {
      ...
      await _importErrorTypeOutlines(db);
      await _importKnowledgeOutlines(db);
  ...
  Future<void> _importErrorTypeOutlines(Database db) async {
  ...
  Future<void> _importKnowledgeOutlines(Database db) async {
  ...
  Future<void> _importErrorRecords(Database db) async {
    final jsonString = await rootBundle.loadString('assets/test_data/error_records.json');
    final data = json.decode(jsonString);
  ...
  Future<void> _importExercises(Database db) async {
    final jsonString = await rootBundle.loadString('assets/test_data/exercises.json');
    final data = json.decode(jsonString);
  ...
  Future<void> _importPortfolioItems(Database db) async {
    final jsonString = await rootBundle.loadString('assets/test_data/portfolio_items.json');
    final data = json.decode(jsonString);
  ...
  Future<void> _importKnowledgePoints(Database db) async {
    final jsonString = await rootBundle.loadString('assets/test_data/knowledge_points.json');
    final data = json.decode(jsonString);
  ...
  Future<bool> exportErrorTypeOutlinesToConfig() async {
    try {
      ...
      final data = {
        'error_type_outlines': outlines.map((o) => {
          'eid': o.eid,
          'content': o.content,
          'lang': o.lang,
        }).toList(),
      ...
      await File(configPath).writeAsString(const JsonEncoder.withIndent('  ').convert(data));
  ...
  Future<bool> exportKnowledgeOutlinesToConfig() async {
    try {
      ...
      final data = {
        'knowledge_outlines': outlines.map((o) => {
          'cid': o.cid,
          'content': o.content,
          'lang': o.lang,
        }).toList(),
